...
import { parsePendingCheck } from '@/lib/heroUtils'
...
function CheckPageInner({ userEmail, plan, initialUsed }: CheckPageClientProps) {
  ...
  useEffect(() => {
    const raw = sessionStorage.getItem('kerfuffle_pending_check')
    if (!raw) return
    sessionStorage.removeItem('kerfuffle_pending_check')
    const pending = parsePendingCheck(raw)
    if (pending) {
      handleCheck(pending.input, pending.inputType)
    }
  ...
  return (
    <div className="relative min-h-screen overflow-hidden">
      ...
      <main className="max-w-4xl mx-auto px-6 py-12 space-y-6">
        ...
        <div className="mb-8">
          <h1 className="font-[family-name:var(--font-serif)] italic text-snow text-4xl mb-2">
            Verify before you deliver.
          </h1>
          <p className="text-muted text-sm">
            Paste AI-generated text or a URL — every factual claim gets checked.
          </p>
        </div>
